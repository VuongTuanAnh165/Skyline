...
<header class="header__section header__transparent">
    <div class="header__topbar bg__primary2">
        <div class="container">
            <div class="header__topbar--inner d-flex align-items-center justify-content-between">
                <a href="{{ route('admin.fe.home.index') }}" target="_blank" class="wellcome__text m-0"><b>Trang dịch
                        vụ</b></a>
                    ...
                    <div class="language__currency d-none d-lg-block">
                        <ul class="d-flex align-items-center">
                            <li class="language__currency--list">
                                <a class="language__switcher" href="{{ route('restaurant.home.index') }}"
                                    target="_blank"><b>Trang quản lý</b></a>
                            </li>
                            <li class="language__currency--list">
                                <a class="account__currency--link" href="{{ route('sell.home.index') }}"
                                    target="_blank"><b>Trang bán hàng</b></a>
                            </li>
                        </ul>
                    </div>
        ...
        <div class="container">
            <div class="main__header--inner position__relative d-flex justify-content-between align-items-center">
                ...
                <div class="main__logo">
                    <h1 class="main__logo--title"><a class="main__logo--link" href="{{ $url['home'] }}"><img
                                class="main__logo--img" src="{{ $url['logo'] }}" alt="logo-img"></a></h1>
                </div>
                <div class="header__menu d-none d-lg-block">
                    <nav class="header__menu--navigation">
                        <ul class="d-flex">
                            @php
                                $arg_home = ['user.food.home.index', 'user.home.index'];
                                $active = '';
                            ...
                            @php
                                $arg_all_product = ['user.food.allProduct.index', 'user.allProduct.index', 'user.food.product.index', 'user.product.index', 'user.food.product.show', 'user.product.show'];
                                $active = '';
                            ...
                            <li class="header__menu--items {{ $active }} mega__menu--items">
                                ...
                                <div class="header__mega--menu row">
                                    @foreach ($url['categoryHome'] as $item)
                                        <div class="col-3">
                                            <a class="header__mega--subtitle {{ $item->id == app('request')->input('categoryHome') ? 'active' : '' }}"
                                                href="#"
                                                onclick="event.preventDefault(); document.getElementById('product-redirect-{{ $item->id }}').submit();">{{ $item->name }}</a>
                                            <form id="product-redirect-{{ $item->id }}"
                                                action="{{ $url['product'] }}" method="GET"
                                                style="display: none;">
                                                <input type="hidden" name="categoryHome"
                                                    value="{{ $item->id }}">
                                            </form>
                                        </div>
                                    @endforeach
                ...
                <div class="header__account">
                    <ul class="d-flex">
                        ...
                        @php
                            $arg_account = ['user.auth', 'user.verify'];
                            $active = '';
                        ...
                        <li class="header__account--items d-none d-lg-block">
                            @if (isset($user))
                                ...
                                <ul class="header__sub--menu sub-menu-acount">
                                    <li class="header__sub--menu__items"><a class="header__sub--menu__link" href="{{$url['myAccount']}}">Tài khoản</a></li>
                                    <li class="header__sub--menu__items">
                                        <a href="#" class="header__sub--menu__link"
                                            onclick="event.preventDefault(); document.getElementById('logout-form-pc').submit();">Đăng
                                            xuất</a>
                                        <form id="logout-form-pc" action="{{ route('user.logout') }}" method="POST"
                                            style="display: none;">
                                            @csrf
                                        </form>
                                    </li>
                            ...
                            @else
                                ...
                                <a class="header__account--btn btn-account {{ $active }}"
            ...
            <nav class="offcanvas__menu">
                            ...
                            @foreach ($url['categoryHome'] as $item)
                                <li class="offcanvas__sub_menu_li">
                                    <a href="#" class="offcanvas__sub_menu_item"
                                        onclick="event.preventDefault(); document.getElementById('product-redirect-1-{{ $item->id }}').submit();">{{ $item->name }}</a>
                                    <form id="product-redirect-1-{{ $item->id }}" action="{{ $url['product'] }}"
                ...
                <div class="offcanvas__account--items header__account--items">
                    @if (isset($user))
                        ...
                        <ul class="header__sub--menu sub-menu-acount">
                            <li class="header__sub--menu__items"><a href="{{$url['myAccount']}}">Tài khoản</a></li>
                            <li class="header__sub--menu__items">
                                <a href="#" class="header__sub--menu__link"
                                    onclick="event.preventDefault(); document.getElementById('logout-form-sp').submit();">Đăng
                                    xuất</a>
                                <form id="logout-form-sp" action="{{ route('user.logout') }}" method="POST"
                                    style="display: none;">
                                    @csrf
                                </form>
                            </li>
                    ...
                    @else
                        <a class="offcanvas__account--items__btn d-flex align-items-center btn-account"
                            data-url="{{ route('user.auth') }}" data-prev="{{ URL::full() }}"
                            href="javascript:void(0)">
                            <span class="offcanvas__account--items__icon">
                                <svg xmlns="http://www.w3.org/2000/svg" width="20.51" height="19.443"
                                    viewBox="0 0 512 512">
    ...
    <div class="offCanvas__minicart">
        <div class="minicart__header ">
            ...
            <p class="minicart__header--desc">{{ $url['name'] }} thêm mới (<span
                    class="minicart__header--desc_span"></span>)</p>
